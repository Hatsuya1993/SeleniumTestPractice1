...
public class Integration extends Base{
    ...
    @Test
    public void checkTheTabForHotelModuleDoc() throws InterruptedException {
        IntegrationPagePOM integrationPagePOM = new IntegrationPagePOM(driver);
        available.clickItem(integrationPagePOM.getHotelsModuleDoc());
        if(driver.getWindowHandles().size() == 2){
            Assert.assertTrue(true);
            logger.info("2 tabs opened for Hotel Module Doc is correct");
        }
        else{
            Assert.fail();
            logger.info("2 tabs opened for Hotel Module Doc is incorrect");
        }
    }

    @Test
    public void checkTheLinkForHotelModuleDoc() throws InterruptedException {
        IntegrationPagePOM integrationPagePOM = new IntegrationPagePOM(driver);
        available.clickItem(integrationPagePOM.getHotelsModuleDoc());
        available.switchTab(1, driver);
        if(driver.getCurrentUrl().contains("standard-hotels-module")){
            Assert.assertTrue(true);
            logger.info("Url for Hotel Module Doc is correct");
        }
        else{
            Assert.fail();
            logger.info("Url for the Hotel Module Doc is incorrect");
        }
    }
    @Test
    public void checkTheTabForToursModuleDoc() throws InterruptedException {
        IntegrationPagePOM integrationPagePOM = new IntegrationPagePOM(driver);
        available.clickItem(integrationPagePOM.getToursModuleDoc());
        if(driver.getWindowHandles().size() == 2){
            Assert.assertTrue(true);
            logger.info("2 tabs opened for Tours Module Doc is correct");
        }
        else{
            Assert.fail();
            logger.info("2 tabs opened for Tours Module Doc is incorrect");
        }
    }

    @Test
    public void checkTheLinkForToursModuleDoc() throws InterruptedException {
        IntegrationPagePOM integrationPagePOM = new IntegrationPagePOM(driver);
        available.clickItem(integrationPagePOM.getToursModuleDoc());
        available.switchTab(1, driver);
        if(driver.getCurrentUrl().contains("modules/tours/viator")){
            Assert.assertTrue(true);
            logger.info("Url for Tours Doc is correct");
        }
        else{
            Assert.fail();
            logger.info("Url for Tours Doc is incorrect");
        }
    }

    @Test
    public void checkTheTabForCarsModuleDoc() throws InterruptedException {
        IntegrationPagePOM integrationPagePOM = new IntegrationPagePOM(driver);
        available.clickItem(integrationPagePOM.getCarsModuleDoc());
        if(driver.getWindowHandles().size() == 2){
            Assert.assertTrue(true);
            logger.info("2 tabs opened for Cars Module Doc is correct");
        }
        else{
            Assert.fail();
    ...
    @Test
    public void checkTheTabForBoatsModuleDoc() throws InterruptedException {
        IntegrationPagePOM integrationPagePOM = new IntegrationPagePOM(driver);
        available.clickItem(integrationPagePOM.getBoatsModule());
        if(driver.getWindowHandles().size() == 2){
            Assert.assertTrue(true);
            logger.info("2 tabs opened for Boats Module Doc is correct");
        }
        else{
            Assert.fail();
            logger.info("2 tabs opened for Boats Module Doc is incorrect");
        }
    }

    @Test
    public void checkTheLinkForBoatsModuleDoc() throws InterruptedException {
        IntegrationPagePOM integrationPagePOM = new IntegrationPagePOM(driver);
        available.clickItem(integrationPagePOM.getBoatsModule());
        available.switchTab(1, driver);
        if(driver.getCurrentUrl().contains("modules/extras/boats-cruises")){
            Assert.assertTrue(true);
            logger.info("Url for Boats Doc is correct");
        }
        else{
            Assert.fail();
    ...
    @Test
    public void checkTheLinkForRentalsModuleDoc() throws InterruptedException {
        IntegrationPagePOM integrationPagePOM = new IntegrationPagePOM(driver);
        available.clickItem(integrationPagePOM.getRentalsModule());
        available.switchTab(1, driver);
        if(driver.getCurrentUrl().contains("modules/extras/rentals")){
            Assert.assertTrue(true);
            logger.info("Url for Rentals Doc is correct");
        }
        else{
            Assert.fail();

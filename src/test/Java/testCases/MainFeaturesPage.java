...
public class MainFeaturesPage extends Base {
    ...
    @Test
    public void securityOptionsFifteenthCheck() throws InterruptedException {
        MainFeaturesPagePOM mainFeaturesPagePOM =
                new MainFeaturesPagePOM(driver);
        Actions.scrollHelperImplicit(500, driver);
        if (Helper.displayed(mainFeaturesPagePOM.getSecurityOptionsSelectorHelper(13))) {
            Assert.assertTrue(true);
            logger.info("Security options selector Fifteenth title displayed");
        } else {
            Assert.fail();
            logger.info("Security options Fifteenth title is not displayed");
        }
    }

    @Test
    public void securityOptionsFifteenthCheckData() throws InterruptedException {
        MainFeaturesPagePOM mainFeaturesPagePOM =
                new MainFeaturesPagePOM(driver);
        Actions.scrollHelperImplicit(500, driver);
        if (Helper.checkText(mainFeaturesPagePOM.getSecurityOptionsSelectorHelper(13), "Integrations")) {
            Assert.assertTrue(true);
            logger.info("Security options selector Fifteenth title is correctly " +
                    "populated");
        } else {
            Assert.fail();
            logger.info("Security options Fifteenth title is not correctly " +
                    "populated");
        }
    }

    @Test
    public void securityOptionsSixteenthCheck() throws InterruptedException {
        MainFeaturesPagePOM mainFeaturesPagePOM =
                new MainFeaturesPagePOM(driver);
        Actions.scrollHelperImplicit(500, driver);
        if (Helper.displayed(mainFeaturesPagePOM.getSecurityOptionsSelectorHelper(14))) {
            Assert.assertTrue(true);
            logger.info("Security options selector Sixteenth title displayed");
        } else {
            Assert.fail();
